...
class Obstacle extends SpriteComponent
  ...
  static const double baseSize = 64.0; // 기준 크기 — 플레이어(80)보다 살짝 작게
  static const double groundEmbed = 6.0; // 이미지 바닥 투명 여백만큼 땅에 살짝 박아 부유감 제거

  /// 크기 배율 (0.8 = 작은 선인장, 1.2 = 큰 선인장)
  final double sizeFactor;

  /// 군집 스폰용 가로 오프셋 — 화면 오른쪽 기준 등장 위치를 뒤로 민다.
  final double xOffset;

  Obstacle({this.sizeFactor = 1.0, this.xOffset = 0})
      : super(
          size: Vector2.all(baseSize * sizeFactor),
  ...
  Future<void> onLoad() async {
    ...
    position = Vector2(
      gameRef.size.x + size.x / 2 + xOffset,
      groundY + groundEmbed * sizeFactor,
    );
    ...
    add(RectangleHitbox(
      ...
      position: Vector2(size.x * 0.225, size.y * 0.08),
    ));
  }
